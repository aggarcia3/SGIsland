using SGIsland.NoiseGenerators;
using SGIsland.Player;
using SGIsland.Util;
using System;
...
namespace SGIsland.TerrainGeneration
{
    ...
    public sealed class IslandTerrainGenerator : MonoBehaviour
    {
        ...
        [SerializeField]
        private GameObject islandWater;
        [SerializeField]
        private GameObject islandBounds;
        [SerializeField]
        private HumanPlayerMovement player;
        [SerializeField]
        ...
        [SerializeField]
        private float _terrainNoiseFrequency = 4;
        [SerializeField]
        private float _terrainNoisePersistence = 0.5f;
        [SerializeField]
        private float _terrainNoiseLacunarity = 1.6f;
        [SerializeField]
        private float _islandRadiusVariance = 0.85f;
        [SerializeField]
        private float _islandShorelineLength = 1;
        [SerializeField]
        private float _minimumHeightAboveSea = 0.15f;
        [SerializeField]
        private Vector2Int _terrainLayerTextureSize = new Vector2Int(512, 512);
        ...
        private void OnValidate()
        {
            ...
            if (!islandWater || !islandBounds || !player || !stars)
                throw new ArgumentException("Either the island water, bounds collider, player or stars game objects are null");
        }
        ...
        public IEnumerator GenerateTerrain(long seed, Action<TerrainGenerationOperation> workUnitDoneCallback)
        {
            ...
            islandBounds.transform.position = new Vector3(0, halfColliderHeight, 0);
            islandBounds.transform.localScale = new Vector3(terrainData.size.x, halfColliderHeight * 2, terrainData.size.z);
            islandBounds.transform.rotation = Quaternion.identity;

            // Spawn the player in free fall in a suitable spot
            player.transform.position = new Vector3(
                player.transform.position.x,
                2 * _maximumTerrainAmplitude * terrainData.heightmapScale.y,
                player.transform.position.z
            );

            // Set player sex according to the seed
            player.IsFemale = seed % 101 > 50;
            ...
            Texture2D sandTexture = new Texture2D(_terrainLayerTextureSize.x, _terrainLayerTextureSize.y, TextureFormat.RGBA32, true);
            ...
            Texture2D grassTexture = new Texture2D(_terrainLayerTextureSize.x, _terrainLayerTextureSize.y, TextureFormat.RGBA32, true);
            ...
            Texture2D dirtTexture = new Texture2D(_terrainLayerTextureSize.x, _terrainLayerTextureSize.y, TextureFormat.RGBA32, true);
            ...
            Texture2D rockTexture = new Texture2D(_terrainLayerTextureSize.x, _terrainLayerTextureSize.y, TextureFormat.RGBA32, true);
        ...
        {
            var sandTextureData = sandTexture.GetRawTextureData<Color32>();
            ...
            for (int x = 0; x < _terrainLayerTextureSize.x; ++x)
            {
                ...
                for (int y = 0; y < _terrainLayerTextureSize.y; ++y, ++i)
                {
                    ...
                    sandTextureData[i] = new Color32(
                        (byte) (Mathf.LerpUnclamped(0.76f, 1, rippleIntensity) * 255),
                        (byte) (Mathf.LerpUnclamped(0.7f, 0.94f, rippleIntensity) * 255),
                        (byte) (Mathf.LerpUnclamped(0.5f, 0.79f, rippleIntensity) * 255),
        ...
        {
            var grassTextureData = grassTexture.GetRawTextureData<Color32>();
            ...
            for (int x = 0; x < _terrainLayerTextureSize.x; ++x)
            {
                ...
                for (int y = 0; y < _terrainLayerTextureSize.y; ++y, ++i)
                {
                    ...
                    grassTextureData[i] = new Color32(
                        // Lagrange polynomials for RGB component interpolation between three base grass colors
                        (byte) ((0.38f * ut * ut - 0.81f * ut + 0.59f) * 255),
                        (byte) ((0.16f * ut * ut - 0.58f * ut + 0.64f) * 255),
                        (byte) ((-0.14f * ut * ut - 0.17f * ut + 0.34f) * 255),
                        0 // No smoothness, so we don't get specular highlights
                    );
                }
        ...
        {
            var dirtTextureData = dirtTexture.GetRawTextureData<Color32>();
            float halfTerrainLayerTextureOffByOneWidth = (float)(_terrainLayerTextureSize.x - 1) / 2;
            float halfTerrainLayerTextureOffByOneHeight = (float)(_terrainLayerTextureSize.y - 1) / 2;
            ...
            for (int x = 0; x < _terrainLayerTextureSize.x; ++x)
            {
                ...
                for (int y = 0; y < _terrainLayerTextureSize.y; ++y, ++i)
                {
                    ...
                    dirtTextureData[i] = new Color32(
                        (byte) (Mathf.LerpUnclamped(0.61f, 0.71f, ut) * 255),
                        (byte) (Mathf.LerpUnclamped(0.47f, 0.57f, ut) * 255),
                        (byte) (Mathf.LerpUnclamped(0.33f, 0.43f, ut) * 255),
                        0 // No smoothness, so we don't get specular highlights
                    );
                }
...
/// <summary>
/// Extends the <see cref="TerrainGenerationOperation"/> enum with useful methods.
/// </summary>
public static class TerrainGenerationOperationExtension
{
    /// <summary>
    /// Returns the terrain generation operation name in an user-friendly form, that may
    /// be shown to the player.
    /// </summary>
    /// <param name="terrainGenerationOperation">The operation to return its user-friendly name name of.</param>
    /// <returns>The user-friendly name of the terrain generation operation.</returns>
    public static string GetUserFriendlyName(this TerrainGenerationOperation terrainGenerationOperation)
    {
        switch (terrainGenerationOperation)
        {
            case TerrainGenerationOperation.TERRAIN_HEIGHTMAP_GENERATION:
                return "Generando mapa de altura de terreno";
            case TerrainGenerationOperation.TERRAIN_HOLE_GENERATION:
                return "Generando mapa de agujeros de terreno";
            case TerrainGenerationOperation.SAND_TEXTURE_GENERATION:
            ...
            case TerrainGenerationOperation.LAYER_AND_ALPHAMAP_GENERATION:
                return "Generando capas y mapas de capas de terreno";
            default:
